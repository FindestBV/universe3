  ...
  return (
    <Flex tag="article" alignItems="stretch" direction="column">
      {COLOR_CARD_NAMES.map((colorName) => {
        const colorKeys = Object.keys(allColors).filter((colorKey) =>
          colorKey.startsWith(colorName.toLowerCase()),
        );
                ...
                .map((key) => (
                  <li key={key}>
                    <Card
                      colorKey={key}
                      colorName={colorName}
                      colorShade={key.split(colorName.toLowerCase())[1]}
                    />
                  </li>
                ))}
            </Grid.Root>
...
const ContrastInfo = ({ backgroundColor = "", isLighter = false, isSmall = false }) => {
  ...
  return (
    <Flex alignItems="stretch" direction="column" flex={1} textAlign="center">
      ...
      >
        {tinycolor2.isReadable(textColor, backgroundColor, {
          level: "AAA",
